...
export default {
  ...
  methods: {
    toShare: function () {
      let shared = {}
      for (let i = 0; i < this.cvShareData.length; i++) {
        if (typeof this[this.cvShareData[i]] !== 'undefined')
          shared[this.cvShareData[i]] = this[this.cvShareData[i]]
      }
      return shared
    },
    add (files) {
      if (files) {
        if (this.files.length && !this.cMultiple) {
          this.reset()
        }
    ...
    __remove (file) {
      ...
      if (this.uploading && !done) {
        this.$emit('cv-remove:abort', this.toShare(), file, file.xhr)
        this.$emit('remove:abort', file, file.xhr)
        file.xhr.abort()
        this.uploadedSize -= file.__uploaded
      } else {
        //custom
        this.$emit(`cv-remove:${done ? 'done' : 'cancel'}`, this.toShare(), file, file.xhr)
      ...
      return new Promise((resolve, reject) => {
        ...
        xhr.onreadystatechange = () => {
          ...
          if (xhr.status && xhr.status < 400) {
            ...
            file.__progress = 100
            //custom
            this.$emit('cv-uploaded', this.toShare(), file, xhr)
            this.$emit('uploaded', file, xhr)
            resolve(file)
          } else {
            file.__failed = true
            //custom
            this.$emit('cv-fail', this.toShare(), file, xhr)
    ...
    upload () {
      ...
      this.xhrs = []
      //
      this.$emit('cv-start', this.toShare())
      this.$emit('start')
      let solved = () => {
        filesDone++
        if (filesDone === length) {
          ...
          this.__computeTotalSize()
          //custom
          this.$emit('cv-finish', this.toShare())
          this.$emit('finish')
        }
      }
      this.queue
    ...
    reset () {
      ...
      this.expanded = false
      this.__computeTotalSize()
      this.$emit('cv-reset', this.toShare())
      this.$emit('reset')
    }
  }
}
